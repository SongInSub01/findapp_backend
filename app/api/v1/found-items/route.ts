...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { message: error instanceof Error ? error.message : '습득물 목록을 불러오지 못했습니다.' },
      { status: 400 },
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    const payload = foundItemSchema.parse(await request.json());
    const item = await createFinderFoundItem(payload);
    return NextResponse.json({ item }, { status: 201 });
  } catch (error) {
    return NextResponse.json(
      { message: error instanceof Error ? error.message : '습득물 등록에 실패했습니다.' },
      { status: 400 },
    );
  }
}
