// 단일 습득물의 상세 조회, 수정, 삭제를 처리하는 API다.
import { NextRequest, NextResponse } from 'next/server';
import { z } from 'zod';

import {
  deleteFinderFoundItem,
...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json(
      { message: error instanceof Error ? error.message : '습득물 상세를 불러오지 못했습니다.' },
      { status: 404 },
    );
  }
}
...
export async function PATCH(
  ...
  try {
    const { itemId } = await context.params;
    const payload = foundItemUpdateSchema.parse(await request.json());
    const item = await updateFinderFoundItem({ ...payload, itemId });
    return NextResponse.json({ item });
  } catch (error) {
    return NextResponse.json(
      { message: error instanceof Error ? error.message : '습득물 수정에 실패했습니다.' },
      { status: 400 },
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    await deleteFinderFoundItem({ loginId: query.loginId, itemId });
    return NextResponse.json({ success: true });
  } catch (error) {
    return NextResponse.json(
      { message: error instanceof Error ? error.message : '습득물 삭제에 실패했습니다.' },
      { status: 400 },
    );
  }
}
