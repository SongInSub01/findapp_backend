...
export async function getBleDeviceByIdAndUserId(deviceId: string, userId: string) {
  const result = await query<{
    id: string;
    user_id: string;
    name: string;
    ...
    `
      select id, user_id, name, icon_key, status, location, last_seen, ble_code,
             map_x, map_y, distance, reward, photo_asset_path
      from ble_devices
      where id = $1
        and user_id = $2
...
export async function updateBleDeviceTrackingState(input: {
  deviceId: string;
  userId: string;
  status: 'safe' | 'lost' | 'contact';
  location: string;
